...
# -*- coding: utf-8 -*-
"""
BUILD_RELEASE.py - Script de Compilación Automática Flutter
==============================================================

Genera dos versiones de la app Strupeni Técnicos:
  - Versión Usuario (MINOR par) -> SIN debug
  - Versión Debug (MINOR impar) -> CON debug
...
def toggle_debug_feature(enable=True):
    """Comenta o descomenta el bloque de debug en home_screen.dart"""
...
def main():
    ...
    try:
        ...
        # VersionApk ya fue creada en el paso anterior
        dst_apk_version = f'VersionApk/{filename_debug}'
        
        if os.path.exists(src_apk):
            # Copiar a build/ y a VersionApk/
            shutil.copy2(src_apk, dst_apk_build)
            shutil.copy2(src_apk, dst_apk_version)
            print_success(f"APK Debug generado y guardado")
            print(f"       📍 build/app/outputs/flutter-apk/{filename_debug}")
            print(f"       📍 VersionApk/{filename_debug}\n")
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("═" * 75 + "\n")
            f.write("  RESUMEN DE COMPILACIÓN - Strupeni Técnicos App\n")
            f.write("═" * 75 + "\n\n")
            ...
            f.write(f"     • Archivo: strupeni-tecnicos-v{debug_version}-DEBUG.apk\n")
        ...
        print(f"    2️⃣  strupeni-tecnicos-v{debug_version}-DEBUG.apk")
        print("        └─ Para desarrollo y testing (CON debug)\n")
        print(f"📍 Ubicaciones:")
